#!/bin/bash
# ==========================================
# Deskhost Premium MOTD Installer (v3 PRO)
# FULL CLEAN + ONLY CUSTOM MOTD
# ==========================================

set -e

echo "🔧 Installing Deskhost Premium MOTD..."
...
# Add ONLY our MOTD
echo "session optional pam_exec.so stdout /etc/update-motd.d/00-deskhost" >> /etc/pam.d/sshd
echo "session optional pam_exec.so stdout /etc/update-motd.d/00-deskhost" >> /etc/pam.d/login

# ================================
# CREATE PREMIUM MOTD SCRIPT
# ================================
echo "✨ Creating Deskhost MOTD..."

cat << 'EOF' > /etc/update-motd.d/00-deskhost
#!/bin/bash
...
# ===== HEADER =====
echo -e "${GREEN}🚀 Welcome to Deskhost Datacenter${RESET}"
...
# ===== FOOTER =====
echo -e "${GREEN}Support:${RESET}  support@deskhost.fun"
echo -e "${GREEN}Discord:${RESET}  https://discord.gg/deskhost"
echo -e "${GREEN}Website:${RESET}  https://deskhost.fun"
echo -e "${MAGENTA}Deskhost — Premium Hosting Experience 💎${RESET}"
echo ""
EOF

chmod +x /etc/update-motd.d/00-deskhost
...
echo ""
echo "✅ Deskhost MOTD Installed (ONLY MODE ENABLED)"
